...
void InitSDL(const char *Name, SDL_Window *&window, SDL_Renderer *&renderer) {
    ...
    SDL_RenderSetScale(renderer, 1.00 * xpix / TK::WWIDTH, 1.00 * ypix / TK::WHEIGHT);
...
bool GameLoop(SDL_Renderer *renderer) {
    ...
    if(ff==1){
        ff=0;
        TK::Boss New(TK::MWIDTH/2+500,TK::MHEIGHT/2+500,2.5,TK::Pi/2,IMG_LoadTexture(renderer, "res/pic/shiteater.png")); 
        boss.push_back(New);
        boss.begin()->hp=50;
